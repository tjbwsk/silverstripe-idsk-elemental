		...
		<div class="idsk-crossroad<% if $Variant %> $Variant<% else %> idsk-crossroad-1<% end_if %>">
			<% if $Variant == 'idsk-crossroad-2' %>
				<% loop $Items %>
					...
					<% include Rasstislav/IdSk/Includes/Components/Crossroad/CrossroadItem HideMobile=$CrossroadIsOneColumnItemHidden, TwoColumnsHide=$CrossroadIsTwoColumnItemHidden, Link=$Link.LinkURL, LinkTitle=$Link.Title, TargetBlank=$Link.OpenInNewWindow %>
				<% end_loop %>
			<% else %>
				<% loop $Items %>
					<% include Rasstislav/IdSk/Includes/Components/Crossroad/CrossroadItem OneColumnHide=$CrossroadIsOneColumnItemHidden, Link=$Link.LinkURL, LinkTitle=$Link.Title, TargetBlank=$Link.OpenInNewWindow %>
				<% end_loop %>
			<% end_if %>
		</div>
	<% end_set %>
